...
class YOLOall(object):
    def __init__(self):
        ...
        self.model_path = './model_data/last10_608.h5'
    ...
    @staticmethod
    def vis(frame, boxs, confidence, class_names):
        """
        ...
        for i in range(len(class_names)):
            cv2.rectangle(frame, (boxs[i][0], boxs[i][1]), (boxs[i][2], boxs[i][3]), (255, 255, 255), 2)
            cv2.putText(frame, str(class_names[i]), (boxs[i][0], int(boxs[i][1] - 20)), 0, 5e-3 * 150, (255, 255, 255),
                        2)  # 类别
            cv2.putText(frame, str(round(confidence[i], 2)), (int(boxs[i][0]), int(boxs[i][1] - 40)), 0, 5e-3 * 150,
                        (255, 255, 255),
                        2)  # 置信度
        return frame
