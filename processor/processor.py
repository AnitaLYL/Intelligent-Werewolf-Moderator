...
class Processor(object):
            ...
            for i in range(len(hand_list)):
                ...
                for j in range(len(self.persons)):
                    try:
                        ...
                        empty_time += 1
                        print('processor: hand为空集 / person 丢失', str(empty_time), '个')
                final_handlist[index_shortest].append(hand_list[i])
        except:
            print('processor: 没有person可以和hand posture关联')
            ...
            for i in range(len(eye_list)):
                ...
                for j in range(len(self.persons)):
                    try:
                        ...
                        empty_time += 1
                        print('processor: eye为空集 / person 丢失', str(empty_time), '个')
                final_eyelist[index_shortest].append(eye_list[i])
        except:
            print('processor: 没有person可以和eye关联')
    ...
    def voting(self):
        ...
        for i in range(1, self.num_player_indevice + 1):
            self.id_hand_status[i] = Counter(self.id_hand_deque[i]).most_common(1)[0][0]
            self.id_eye_status[i] = Counter(self.id_eye_deque[i]).most_common(1)[0][0]
